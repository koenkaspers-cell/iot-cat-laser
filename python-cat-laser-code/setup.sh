...
# ── 1. System update ──────────────────────────────────
echo ""
echo "[1/$TOTAL_STEPS] Updating package lists..."
sudo apt update

# ── 2. System dependencies ────────────────────────────
echo ""
echo "[2/$TOTAL_STEPS] Installing system dependencies..."
sudo apt install -y \
    python3-venv \
    python3-full \
    libcap-dev \
...
echo ""
echo "[3/$TOTAL_STEPS] Enabling camera, I2C and SPI interfaces..."
...
# ── 4. pigpio daemon ───────────────────────────────────
echo ""
echo "[4/$TOTAL_STEPS] Enabling pigpio daemon (required for servo PWM)..."
sudo systemctl enable pigpiod
sudo systemctl start pigpiod

# ── 5. Python virtual environment ─────────────────────
echo ""
echo "[5/$TOTAL_STEPS] Creating Python virtual environment..."
...
# ── 6. Python packages ─────────────────────────────────
echo ""
echo "[6/$TOTAL_STEPS] Installing Python packages into venv..."
"$VENV_DIR/bin/pip" install --upgrade pip
"$VENV_DIR/bin/pip" install flask flask-socketio pigpio

# ── 7. Verify RTSP stream works ───────────────────────
echo ""
echo "[7/$TOTAL_STEPS] Verifying libcamera-vid is available for RTSP..."
if command -v libcamera-vid &> /dev/null; then
    echo "      libcamera-vid found — RTSP streaming ready."
else
    echo "      WARNING: libcamera-vid not found."
    echo "      Try: sudo apt install libcamera-apps"
fi

# ── 8. Project files ───────────────────────────────────
echo ""
echo "[8/$TOTAL_STEPS] Setting up project directory at $PROJECT_DIR ..."
mkdir -p "$PROJECT_DIR"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cp "$SCRIPT_DIR/catlaser.py"    "$PROJECT_DIR/"
cp "$SCRIPT_DIR/calibration.py" "$PROJECT_DIR/"
cp "$SCRIPT_DIR/vitals.py"      "$PROJECT_DIR/"
...
echo ""
echo "[9/$TOTAL_STEPS] Installing systemd service so the server auto-starts on boot..."
...
echo ""
echo "  Architecture:"
echo "    RTSP stream:   rtsp://[tailscale-ip]:8554/stream  (video)"
echo "    Socket.IO:     http://[tailscale-ip]:5001          (servo/laser control)"
echo ""
...
echo "    python3 $PROJECT_DIR/vitals.py --watch  # monitor Pi health"
echo ""
